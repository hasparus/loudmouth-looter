...
// ESLint-8 vintage (needs fixupPluginRules); better-tailwindcss and check-file
// are modern and flat-config-native.
...
export default [
  ...
  // SolidJS rules (ports `plugin:solid/typescript` from the old .eslintrc.cjs).
  // The site is SolidJS; these catch reactivity mistakes the React plugins miss.
  {
    files: ["**/*.ts", "**/*.tsx"],
  ...
  // Tailwind class validation (ported from .eslintrc.cjs).
  {
    files: ["**/*.tsx", "**/*.astro"],
  ...
  // Enforce .tsx-only source files (ported from .eslintrc.cjs).
  {
    files: ["src/**/*.ts"],
